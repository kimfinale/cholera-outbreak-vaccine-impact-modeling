...
1. Exclude outbreaks that have already been affected by OCVs
2. Exclude outbreaks for which mortality data are not available
3. Exclude outbreaks for which economic variables are not available (workforce, GDP, etc.)
4. Exclude outbreaks reported to have occurred in a population over 3M as more than 5M doses are not likely to be approved, based on the records from XX through XX. 


```{r}
summary(ds$population)
summary(ds[ds$ID %in% ts824_wk$data_id, ]$population)
ds824 <- ds[ds$ID %in% ts824_wk$data_id, ]
summary(ds824$population)
unique(ds824[ds824$population > 3e6,]$ID_outbreak)
```
...
ive_yhat_yrep <- list(
  yhat =  as.data.frame(rstan::extract(fit)$new_mu),
  yrep = as.data.frame(rstan::extract(fit)$yrep),
  vacc_cov = round(expit(rstan::extract(fit)$new_X[1,,2]), digits=2))

ive_yhat_yrep$vacc_cov
saveRDS(ive_yhat_yrep, paste0("outputs/ive_yhat_yrep_", tstamp(), ".rds"))
...
## Global stockpile deployment data

Data were manually extracted from the dashboard.
```{r}
