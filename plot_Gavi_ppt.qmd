...
svim1_week |>
  ...
  mutate(sCh_vacc = sCh_tot - sCh_averted_tot,
         outbk_threshold = ifelse(sCh_vacc > threshold, 1, 0)) -> svim1_week
...
svim1_week_summary <- svim1_week |> 
  ...
  summarise(data_id = data_id[1],
            ...
            outbk_dur = n(),
            outbk_dur_vacc = sum(outbk_threshold),
            pct_reduc_dur = 100 * (outbk_dur - outbk_dur_vacc) / outbk_dur)

svim1 <- 
  left_join(svim1, 
            svim1_week_summary[,c("id", "week_vaccination","vacc_cov",
                                  "pct_reduc_case_wk","pct_reduc_dur")],
...
## Figures - Vaccine impact summary 
...
svim1 |> 
  ...
  scale_color_manual("Vaccine coverage", values=c(set1[3],set1[2]), labels=c("60%","90%"))+
  scale_y_continuous(labels = scales::percent)+
  theme_light()+
...
# fac <- 1.25
# ggsave(paste0("plots/pct_case_red_camp_timing_", tstamp(), ".pdf"), plt,
#        device = cairo_pdf, width=fgw1*fac, height=fgw1*fac, units="mm")
...
# fac <- 1.25
# ggsave(paste0("plots/pct_case_reduc_vc_60_90_camp_timing_", tstamp(), ".pdf"), plt,
#        device = cairo_pdf, width=fgw1*fac, height=fgw1*fac, units="mm")
